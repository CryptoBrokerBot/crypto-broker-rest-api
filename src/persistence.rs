...
impl CryptoPortfolioDao {

  pub fn new(ds : &DataSource) -> CryptoPortfolioDao {
    return CryptoPortfolioDao{config : ds.into()};
  }
  ...
  pub async fn get_latest_price<S : AsRef<str>>(&self, crypto_id : S) -> StdResult<Numeric> {
    let query = r#"
    SELECT price FROM cryptodata where LOWER(cryptoId) = LOWER($1) ORDER BY asOf DESC LIMIT 1;
    "#;
    let config = self.config.clone();
    let client = get_client!(config);
    let price : Numeric = client.query_one(query,&[&crypto_id.as_ref()]).await?.try_get("price")?;
    Ok(price)
  }
  ...
  pub async fn get_portfolio<S : AsRef<str>>(&self, user_id : &S) -> StdResult<Portfolio> {
    let conf = self.config.clone();
    let client : Client = get_client!(conf);
    let balance = self.get_wallet_balance(user_id).await?;
  ...
  pub async fn update_server_patrons<S : AsRef<str>>(&self, user_id : &S, server_id : &S) -> StdResult<()> {
    let conf = self.config.clone();
    let client : Client = get_client!(conf);
    Ok(client.execute("INSERT INTO serverpatrons (userId,serverId) VALUES ($1,$2)",&[&user_id.as_ref(),&server_id.as_ref()]).await.map(|_| ())?)
  }

  pub async fn create_wallet<S : AsRef<str>>(&self, user_id : S, balance : Numeric) -> StdResult<()> {
    let query = r#"
        INSERT INTO wallets (userId, walletbalance) VALUES($1,$2);
    "#;
    let config = self.config.clone();
    let client : Client = get_client!(config);
    client.execute(query, &[&user_id.as_ref(),&balance]).await?;
    Ok(())
  }
}
