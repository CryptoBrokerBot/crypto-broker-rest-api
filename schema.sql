...
WITH cteLatestPrices AS (
  SELECT symbol, price as latestPrice FROM (SELECT symbol, price, ROW_NUMBER() OVER(partition by symbol order by asOf DESC) as rn
...
lastTransactionTs
FROM ctePositions p
JOIN cteLatestPrices lp on p.symbol = lp.symbol
